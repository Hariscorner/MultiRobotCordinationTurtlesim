...
int main(int argc, char **argv) {
	...
	turtlesim::Spawn::Request req[2];
	turtlesim::Spawn::Response resp[2];
	turtlesim::Pose TPose[2];
	...
	ros::Subscriber curr_pose_sub = nh.subscribe<turtlesim::Pose>("/"+req[0].name+"/pose", 2, boost::bind(currPoseCallback, _1,0, TPose));	// subscriber objects to get current pose of each turtle
	
	ros::Publisher turtle_vel_pub = nh.advertise<geometry_msgs::Twist>("/"+req[0].name+"/cmd_vel", 2);	//publisher objects to publish the command velocity
...
void spawn_my_turtles(ros::NodeHandle& nh, turtlesim::Spawn::Request req[], turtlesim::Spawn::Response resp[]) {
	...
	//req.name	= "myturtle1";
	ros::param::get("~T1Name",		req[0].name);
	ros::param::get("~T1X",		req[0].x);
	ros::param::get("~T1Y",		req[0].y);
	ros::param::get("~T1Theta",	req[0].theta);
	req[0].theta=req[0].theta * M_PI/180;
	
	bool success=spawnTurtle.call(req[0],resp[0]);
